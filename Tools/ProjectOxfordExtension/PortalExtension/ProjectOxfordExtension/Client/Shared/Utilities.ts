...
export function getBaseImage(id: string) {
	switch (id) {
...
export function getDisabledSpecsByApiType(data: any, apiType: any, skuQuota: any): Array<HubsExtension.Azure.SpecPicker.DisabledSpec> {
		...
		if (data.value().filter(function (account: any, index: number, accounts: any) {
			...
			specsToDisable.push({
				...
				helpBalloonMessage: ClientResources.SpecCard.disabledHelpMessage.format(
					skuQuota[i].quota,
					getPricingText(skuQuota[i].code))
			});
		}
	}
