...
define(["require", "exports", "ClientResources", "Shared/Constants", "../_generated/SvgLogo"], function (require, exports, ClientResources, Constants, Svg) {
    ...
    function getBaseImage(id) {
        switch (id) {
            case "BaseImages.Info":
                return BaseImages.Info();
            case "BaseImages.Publish":
                return BaseImages.Publish();
            case "BaseImages.Tools":
                return BaseImages.Tools();
        }
    }
    ...
    function getDisabledSpecsByApiType(data, apiType, skuQuota) {
        var specsToDisable = new Array();
        for (var i = 0; i < skuQuota.length; i++) {
            if (data.value().filter(function (account, index, accounts) {
                return accounts[index].kind() == apiType && accounts[index].sku().name() == skuQuota[i].code;
            }).length >= skuQuota[i].quota) {
                specsToDisable.push({
                    specId: skuQuota[i].code,
                    message: ClientResources.SpecCard.disabledMessage,
                    helpBalloonMessage: ClientResources.SpecCard.disabledHelpMessage.format(skuQuota[i].quota, getPricingText(skuQuota[i].code))
                });
            }
        }
