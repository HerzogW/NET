...
export abstract class CreateBladeViewModel extends Forms.Form.ViewModel<CreateModel> {
    ...
    //private _apiItemsView: MsPortalFx.Data.EntityView<any, any>;
    private _apiItemView: MsPortalFx.Data.EntityView<any, any>;
    ...
    constructor(container: MsPortalFx.ViewModels.ContainerContract, initialState: any, dataContext: ResourceArea.DataContext,
        ...
        super(container); console.log("this is current display language: " + Constants.CurrentLanguage);
        ...
        //this._apiItemsView = dataContext.createData.apiItemEntities.createView(container, { interceptNotFound: false });
    ...
    private _mapIncomingDataForEditScope(incoming: CreateModel): CreateModel {
        ...
        if (parameterConfig && parameterConfig.apitype) {
            ...
            this._apiItemView.fetch(apiInfo.name).then(() => {
                var apiItem = this._apiItemView.item();
                if (apiItem) {
                    apiInfo.title = apiItem.item();
                }
            });
    ...
    private _configResourceExistsValidationChain(): void {
        this.subscriptionDropDown.value.subscribe(this._container, (newValue: MsPortalFx.Azure.Subscription) => {
            ...
            if (newValue) {
                ...
                if (this.apiTypeSelector.value() && this.apiTypeSelector.value().name) {
                    this._apiItemView.fetch(this.apiTypeSelector.value().name).then(() => {
                        var apiItem = this._apiItemView.item();
                        if (apiItem) {
                            var skuQuota = ko.toJS(apiItem).skuQuota;
                            if (this.accountSpecSelector.control.value() &&
                                this.accountSpecSelector.control.value().selectedSpecId &&
                                skuQuota.filter((sku: any, index: number, skus: any) => { return skus[index].code == this.accountSpecSelector.control.value().selectedSpecId })[0]) {
                                ...
                                var promise = this._resourcesPerSubscriptionView.fetch(newValue.subscriptionId).then(() => {
                                    if (this._resourcesPerSubscriptionView.item()) {
                                        var specsToDisable = Utilities.getDisabledSpecsByApiType(this._resourcesPerSubscriptionView.item(), this.apiTypeSelector.value().name, skuQuota);
        ...
        this.apiTypeSelector.value.subscribe(this._container, (newValue: ApiTypePart.ApiInfo) => {
            if (newValue) {
                this._apiItemView.fetch(newValue.name).then(() => {
                    var apiItem = this._apiItemView.item();
                    if (apiItem) {
                        this.showLegalTerms(apiItem.showLegalTerm());
                    }
                    else {
